  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div className="w-full max-w-2xl bg-[#0f1829] border border-white/8 rounded-[28px] shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/6 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-white text-sm">Handleiding</p>
                  <p className="text-xs text-white/35">DE GEMEENSCHAP Admin</p>
                </div>
              </div>
              <button onClick={close} className="text-white/25 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 p-6">
              ...
              {tab === 'quickstart' && (
                <div className="space-y-4">
                  <p className="text-sm text-white/40 mb-5">Klik op een taak om de stappen te zien.</p>
                  {QUICK_STEPS.map((task, i) => (
                    <div key={task.id} className="bg-white/4 border border-white/6 rounded-2xl overflow-hidden">
                      <button
                      ...
                      >
                        ...
                        <ChevronDown className={`w-4 h-4 text-white/30 transition-transform ${activeTask === i ? 'rotate-180' : ''}`} />
                      </button>
                      {activeTask === i && (
                  ...
                  <Link
                    ...
                    onClick={close}
                    className="flex items-center justify-center gap-2 w-full mt-4 bg-primary/8 border border-primary/20 text-primary text-sm font-semibold py-3 rounded-xl hover:bg-primary/15 transition-colors"
                  >
              ...
              {tab === 'faq' && (
                <div className="space-y-2">
                  {FAQS.map((item, i) => (
                    <div key={i} className="bg-white/4 border border-white/6 rounded-xl overflow-hidden">
                      <button
                        className="w-full flex items-center justify-between px-5 py-4 text-left gap-3"
                        onClick={() => setOpenFaq(openFaq === i ? null : i)}
                      >
                        <span className="font-semibold text-white/80 text-sm leading-snug">{item.q}</span>
                        <ChevronDown className={`w-4 h-4 text-white/25 shrink-0 transition-transform ${openFaq === i ? 'rotate-180' : ''}`} />
                      </button>
                      {openFaq === i && (
              ...
              {tab === 'contact' && (
                <div className="space-y-4">
                  <div className="bg-white/4 border border-white/6 rounded-2xl p-5">
                    <p className="text-sm font-bold text-white mb-1">📧 E-mail support</p>
                    <p className="text-sm text-white/45 mb-3">Voor technische vragen of problemen met de app.</p>
                    <a href="mailto:info@degemeenschap.be" className="text-primary text-sm font-semibold hover:underline">
                      info@degemeenschap.be
                    </a>
                  </div>
                  <div className="bg-white/4 border border-white/6 rounded-2xl p-5">
                    <p className="text-sm font-bold text-white mb-1">🌐 Website</p>
                    <p className="text-sm text-white/45 mb-3">Meer informatie over vzw De Gemeenschap.</p>
                    <a href="https://degemeenschap.be" target="_blank" rel="noopener noreferrer" className="text-primary text-sm font-semibold hover:underline flex items-center gap-1">
                      degemeenschap.be <ExternalLink className="w-3 h-3" />
                    </a>
                  </div>
                  <div className="bg-white/4 border border-white/6 rounded-2xl p-5">
                    <p className="text-sm font-bold text-white mb-1">📖 Handleiding</p>
                    <p className="text-sm text-white/45 mb-3">Uitgebreide stap-voor-stap instructies.</p>
                    <Link href="/admin/handleiding" onClick={close} className="text-primary text-sm font-semibold hover:underline flex items-center gap-1">
